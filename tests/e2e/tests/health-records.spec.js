...
test.describe('Health Records Management', () => {
  ...
  test('should validate new health record rules - no auto-correction', async ({ page }) => {
    // Test for new validation rules: BP 30-250, HR 30-150, systolic > diastolic
    ...
    // Verify the invalid value is preserved (not auto-corrected)
    await expect(page.locator('input[placeholder="收缩压"]')).toHaveValue('25');
    
    // Test heart rate out of new range (160 should be invalid, not auto-corrected to 150)
    await page.fill('input[placeholder="收缩压"]', '120');
    await page.fill('input[placeholder="舒张压"]', '80');
    ...
    // Verify the invalid value is preserved (not auto-corrected)
    await expect(page.locator('input[placeholder="心率"]')).toHaveValue('160');
    
    // Test systolic <= diastolic relationship
    await page.fill('input[placeholder="收缩压"]', '80');
