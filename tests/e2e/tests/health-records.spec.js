...
test.describe('Health Records Management', () => {
  ...
  test('should filter records by date range', async ({ page }) => {
    ...
    await page.locator(`text=${weekAgo.getDate()}`).first().click();
    await page.locator(`text=${today.getDate()}`).last().click();
    
    // Apply filter
    await page.click('text=应用筛选');
    
    // Verify filter is applied
    await expect(page.locator('.ant-table-tbody tr')).toHaveCount(0); // Should be empty for new test
  });
