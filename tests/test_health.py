...
class TestHealthEndpoints:
    ...
    def test_timestamp_timezone_roundtrip(self, client, auth_headers):
        access_headers = auth_headers['access']

        record_data = {
            'systolic': 118,
            'diastolic': 76,
            'timestamp': '2025-09-26T10:00:00+08:00',
            'tags': [],
        }
        ...
        assert created['timestamp'] == '2025-09-26T02:00:00Z'

        rec_id = created['id']
        fetched = client.get(f'/api/v1/health/{rec_id}', headers=access_headers)
        assert fetched.status_code == 200
        fetched_json = fetched.get_json()
        assert fetched_json['timestamp'] == '2025-09-26T02:00:00Z'

        # Ensure list respects offsets in filters
        date_from = quote('2025-09-26T00:00:00+08:00', safe='')
        date_to = quote('2025-09-26T23:59:59+08:00', safe='')
        response = client.get(f'/api/v1/health?date_from={date_from}&date_to={date_to}', headers=access_headers)
        assert response.status_code == 200
        items = response.get_json()['records']
        assert any(item['id'] == rec_id for item in items)
        assert all(r['timestamp'].endswith('Z') for r in items)
