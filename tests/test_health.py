...
class TestHealthValidation:
    """Test validation rules for health records. Generated by Copilot for issue #5"""
    
    def test_blood_pressure_range_validation(self, client, auth_headers):
        """Test blood pressure range validation (30-250)"""
        access_headers = auth_headers['access']
        
        # Test systolic too low
        record_data = {'systolic': 29, 'diastolic': 80}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 400
        assert "血压范围需在 30-250" in response.get_json()['message']
        
        # Test systolic too high
        record_data = {'systolic': 251, 'diastolic': 80}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 400
        assert "血压范围需在 30-250" in response.get_json()['message']
        
        # Test diastolic too low
        record_data = {'systolic': 120, 'diastolic': 29}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 400
        assert "血压范围需在 30-250" in response.get_json()['message']
        
        # Test diastolic too high
        record_data = {'systolic': 120, 'diastolic': 251}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 400
        assert "血压范围需在 30-250" in response.get_json()['message']
    
    def test_heart_rate_range_validation(self, client, auth_headers):
        """Test heart rate range validation (30-150, optional)"""
        access_headers = auth_headers['access']
        
        # Test heart rate too low
        record_data = {'systolic': 120, 'diastolic': 80, 'heart_rate': 29}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 400
        assert "心率范围需在 30-150" in response.get_json()['message']
        
        # Test heart rate too high
        record_data = {'systolic': 120, 'diastolic': 80, 'heart_rate': 151}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 400
        assert "心率范围需在 30-150" in response.get_json()['message']
        
        # Test heart rate null (should be allowed)
        record_data = {'systolic': 120, 'diastolic': 80, 'heart_rate': None}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 201
        
        # Test heart rate missing (should be allowed)
        record_data = {'systolic': 120, 'diastolic': 80}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 201
    
    def test_systolic_greater_than_diastolic(self, client, auth_headers):
        """Test that systolic must be greater than diastolic"""
        access_headers = auth_headers['access']
        
        # Test systolic equal to diastolic (should fail)
        record_data = {'systolic': 120, 'diastolic': 120}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 400
        assert "收缩压必须大于舒张压" in response.get_json()['message']
        
        # Test systolic less than diastolic (should fail)
        record_data = {'systolic': 80, 'diastolic': 120}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 400
        assert "收缩压必须大于舒张压" in response.get_json()['message']
        
        # Test systolic greater than diastolic (should succeed)
        record_data = {'systolic': 121, 'diastolic': 120}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 201
    
    def test_boundary_values(self, client, auth_headers):
        """Test boundary values for validation"""
        access_headers = auth_headers['access']
        
        # Test minimum valid values
        record_data = {'systolic': 31, 'diastolic': 30, 'heart_rate': 30}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 201
        
        # Test maximum valid values
        record_data = {'systolic': 250, 'diastolic': 249, 'heart_rate': 150}
        response = client.post('/api/v1/health', json=record_data, headers=access_headers)
        assert response.status_code == 201
    
    def test_update_validation(self, client, auth_headers):
        ...
        # Try to update with invalid systolic
        update_data = {'systolic': 29}
        response = client.put(f'/api/v1/health/{record_id}', json=update_data, headers=access_headers)
        assert response.status_code == 400
        assert "血压范围需在 30-250" in response.get_json()['message']
        
        # Try to update creating invalid relationship
        update_data = {'diastolic': 121}  # This would make diastolic > systolic
        response = client.put(f'/api/v1/health/{record_id}', json=update_data, headers=access_headers)
        assert response.status_code == 400
        assert "收缩压必须大于舒张压" in response.get_json()['message']
        ...
        response = client.put(f'/api/v1/health/{record_id}', json=update_data, headers=access_headers)
        assert response.status_code == 200
        assert response.get_json()['systolic'] == 130
