...
## Overview
This document outlines the branch and environment conventions that **must** be followed for the Health Platform project. The policy below is the single source of truth for day-to-day branching, pull requests, and deployment triggers.

## Mandatory Branch Policy

- **Default working branch: `develop`** – all feature work must start from the latest `develop` branch. The legacy `base` branch is no longer used for active development.
- **Feature branches** follow `feature/<issue-number>-<slug>` naming (e.g., `feature/59-wechat-login`). Emergency fixes use `hotfix/<slug>`.
- **Merge path**:
  1. `feature/*` → `develop`
  2. `develop` → `staging`
  3. `staging` → `main`
  Each merge requires PR review plus automated checks for that stage.
- **Deployments**: pushing to `develop`, `staging`, or `main` automatically deploys to their corresponding environments. Feature branches never auto-deploy.
- **Rebasing**: before opening a PR, rebase onto the latest `develop` to avoid drift. Resolve conflicts locally – never merge `develop` into `feature/*` via GitHub UI.
- **Hotfix flow**: branch from `main`, apply the fix, merge back to `main`, then fast-forward `staging` and `develop` to keep histories aligned.

These rules apply immediately and supersede any earlier informal branching habits.
...
### Strategy 1: Branch-Based Single Workflow (Current and Recommended)

#### Branch-Environment Mapping:
- `main` → Production Environment (protected, deployment requires approval)
