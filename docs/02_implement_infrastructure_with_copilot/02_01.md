...
You will be deploying the Health Platform solution for JumpStarX to Azure as part of this training lab. To prepare the infrastructure you will begin by creating issues and generating code from those issues to create the necessary resources.

## Description

During this task you will create a GitHub issue using Chat with Copilot. This issue will be later used to generate a Bicep script to produce the necessary Azure resources (App Service for Containers, ACR, monitoring, and Foundry) for hosting the Health Platform application for JumpStarX.
...
### 02: Open Chat with Copilot on Github.com 
...
Create a GitHub issue using Copilot Chat that describes the infrastructure requirements for deploying the Health Platform web application, including a Linux App Service, Azure Container Registry, Application Insights, and Microsoft Foundry.
...
1. From Chat with Copilot enter the following prompt to create an issue:
    ...
    ```bash
    /create-issue Provision Azure infrastructure for the Health Platform web
    application used by Health Platform. It will need a Linux App Service, and I want to use Docker to deploy to the App Service, so I'll need a Container Registry as well.  But I
    ...
    Application Insights. I will be using GPT-4.1 so I will need Microsoft
    ...
    is a dev environment and for the dev environment, the resource group name is expected as rg-health-platform-dev. I want to use AZD with Bicep to define and deploy
    everything. for the other    environments later, we can create other issues.
    ```
    and for K8S version

       ```bash
    /create-issue Provision Azure infrastructure for the Health Platform web
    application used by Health Platform. It will need a K8S clusgter,and docker registry as well. 
    for K8S cluster, I prefer AKS. and let's start with version 1.26, the node should be linux based, 1 node with 2 vcpu and 8gb memory. The docker registry should be Azure Container Registry. 
    The K8S cluster should use Azure RBAC to pull images not passwords. I want to monitor it with
    Application Insights. I will be using GPT-4.1 so I will need Microsoft
    Foundry, I want to use westus3 because they are available there. Everything
    should be deployed together in one resource group in the same region. for the dev environment, the resource group name is expected as rg-health-platform-dev. I want to use AZD with Bicep to define and deploy
    everything. for the other environments later, we can create other issues.
    ```
...
```text
## Summary
Provision all Azure resources as infrastructure for the Health Platform web
application for JumpStarX (dev environment) into a single resource group in westus3. Use
...
Insights. Microsoft Foundry will also be provisioned for GPT-4.1 access in
westus3 as these models are available there.

## Requirements
- All resources provisioned into a single resource group (e.g. `rg-health-platform-dev-westus3`) in westus3
...
You've completed this task. You have generated an issue with GitHub Copilot that describes the infrastructure requirements for deploying the Health Platform solution for JumpStarX.