...
Use GitHub Copilot Chat (with GitHub MCP and Azure MCP) to plan, design, and deploy Azure infrastructure for the Health Platform web application based on the GitHub issue you created in the previous task. Copilot will generate Bicep + Azure Developer CLI (AZD) infrastructure, help debug issues, and guide fixes when errors occur.
...
4. Prompt Copilot to generate the script
    ...
    please take into consider that health platform is a web application that need both backend. the frontend code is in the folder “frontend” and the backend code is in the folder “src”. the backend and frontend will be in a same container for the dev environment. Please generate bicep scripts and azure developer cli files to deploy the infrastructure needed for this application in azure. Please follow best practices from microsoft cloud practices (MCP) for azure infrastructure as code.
    ```
...
3. The  application will be detected in folder src. Click enter to confirm and continue initializing the app, then confirm the port.
