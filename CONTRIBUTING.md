...
- **代码与注释**：统一使用英文
- **协作沟通**：使用中文
- **项目文档（Markdown）**：默认使用中文（包含 `docs/`、根目录说明文档等）
- **多语言文档**：如需英文版本，采用中英双语，并保证中文内容完整且优先维护
- **提交标注**：每个commit,都需要有comments，如 `` 标记
...
- 通过 GitHub Actions 实现自动化测试、构建、预发布部署：
   - pull request → 自动构建镜像并部署到 **staging/测试环境**，供 QA 和回归验证；仅仓库内分支的 PR 会触发部署（避免 fork 泄露机密）。
   - main/staging/dev 分支 push（或手动触发）→ 按分支策略部署到对应环境；main 仍对应生产。
   - 部署前先跑迁移：迁移脚本需随 PR 提交并过审，流水线在部署前执行 `flask db upgrade`，确保旧数据不丢失。
