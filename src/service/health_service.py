...
def _format_timestamp(dt):
    """Format datetime to ISO string with Z suffix. Generated by Copilot"""
    if dt is None:
        return None
    iso_str = dt.isoformat()
    # If already has timezone info, replace +00:00 with Z
    if iso_str.endswith('+00:00'):
        return iso_str.replace('+00:00', 'Z')
    # If naive datetime (from database), assume UTC and add Z
    elif 'T' in iso_str and not iso_str.endswith('Z'):
        return iso_str + 'Z'
    return iso_str
...
def _validate_health_record_data(data):
    """
    Validate health record data according to business rules.
    Returns (is_valid, errors_dict) where errors_dict contains field-specific errors.
    ...
    try:
        ...
        if not (30 <= systolic <= 250):
            errors["systolic"] = "Blood pressure must be between 30-250 mmHg"
    except (TypeError, ValueError):
        errors["systolic"] = "Invalid systolic blood pressure"
        systolic = None
    ...
    try:
        ...
        if not (30 <= diastolic <= 250):
            errors["diastolic"] = "Blood pressure must be between 30-250 mmHg"
    except (TypeError, ValueError):
        errors["diastolic"] = "Invalid diastolic blood pressure"
        diastolic = None
    
    # Validate systolic > diastolic relationship
    if systolic is not None and diastolic is not None and systolic <= diastolic:
        errors["blood_pressure"] = "Systolic pressure must be greater than diastolic pressure"
    
    # Validate heart rate (optional field)
    heart_rate = data.get("heart_rate")
    if heart_rate is not None and heart_rate != "":
        try:
            heart_rate = int(heart_rate)
            if not (30 <= heart_rate <= 150):
                errors["heart_rate"] = "Heart rate must be between 30-150 bpm"
        except (TypeError, ValueError):
            errors["heart_rate"] = "Invalid heart rate"
...
def create_record():
    ...
    is_valid, validation_errors = _validate_health_record_data(data)
    if not is_valid:
        return jsonify(error("400", "Validation failed", validation_errors)), 400
...
def update_record(rec_id: int):
    ...
    # Create update data with current values as defaults for validation
    update_data = {
        "systolic": data.get("systolic", rec.systolic),
        "diastolic": data.get("diastolic", rec.diastolic),
        "heart_rate": data.get("heart_rate", rec.heart_rate) if "heart_rate" in data else rec.heart_rate
    }
    
    # Validate the update data
    is_valid, validation_errors = _validate_health_record_data(update_data)
    if not is_valid:
        return jsonify(error("400", "Validation failed", validation_errors)), 400
