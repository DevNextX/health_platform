...
def create_record():
    ...
    if not (30 <= systolic <= 250) or not (30 <= diastolic <= 250):
        return jsonify(error("400", "Blood pressure out of range (30-250)")), 400
    if systolic <= diastolic:
        return jsonify(error("400", "Systolic pressure must be greater than diastolic pressure")), 400
    ...
    if heart_rate is not None:
        ...
        if not (30 <= heart_rate <= 150):
            return jsonify(error("400", "Heart rate out of range (30-150)")), 400
...
def update_record(rec_id: int):
    ...
    if "diastolic" in data:
        ...
        if not (30 <= rec.diastolic <= 250):
            return jsonify(error("400", "Blood pressure out of range (30-250)")), 400
    # Validate systolic > diastolic after both are potentially updated
