...
def create_record():
    ...
    # Validation updated: systolic/diastolic 30-250, systolic>diastolic, heart_rate optional 30-150. Generated by Copilot
    def _parse_int(field_name: str, raw):
        try:
            return int(raw)
    ...
    try:
        systolic = _parse_int("systolic", data.get("systolic"))
        diastolic = _parse_int("diastolic", data.get("diastolic"))
    except ValueError as e:
        return jsonify(error("400", str(e))), 400

    if not (30 <= systolic <= 250) or not (30 <= diastolic <= 250):
        return jsonify(error("400", "Blood pressure range must be 30-250")), 400
    if systolic <= diastolic:
        return jsonify(error("400", "Systolic must be greater than diastolic")), 400
    ...
    if heart_rate in ("", None):
        ...
        if not (30 <= heart_rate <= 150):
            return jsonify(error("400", "Heart rate range must be 30-150")), 400
...
def update_record(rec_id: int):
    ...
    if "diastolic" in data:
        ...
        if not (30 <= rec.diastolic <= 250):
            return jsonify(error("400", "Blood pressure range must be 30-250")), 400
    # Relation check only after potential updates of both values
    if ("systolic" in data) or ("diastolic" in data):
        if rec.systolic is None or rec.diastolic is None:
            return jsonify(error("400", "Missing blood pressure values")), 400
        if rec.systolic <= rec.diastolic:
            return jsonify(error("400", "Systolic must be greater than diastolic")), 400
    if "heart_rate" in data:
        hr_raw = data["heart_rate"]
        if hr_raw in ("", None):
            rec.heart_rate = None
        else:
            try:
                rec.heart_rate = int(hr_raw)
            except (TypeError, ValueError):
                return jsonify(error("400", "Invalid heart_rate")), 400
            if not (30 <= rec.heart_rate <= 150):
                return jsonify(error("400", "Heart rate range must be 30-150")), 400
    if "tags" in data:
        tags = data.get("tags") or []
        if not isinstance(tags, list):
