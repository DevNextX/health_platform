...
def create_record():
    ...
    try:
        systolic_raw = data.get("systolic")
        if isinstance(systolic_raw, float) and not systolic_raw.is_integer():
            validation_errors.append({"field": "systolic", "message": "Systolic pressure must be a whole number"})
            systolic = None
        else:
            systolic = int(systolic_raw)
    except (TypeError, ValueError):
        validation_errors.append({"field": "systolic", "message": "Systolic pressure must be a number"})
        systolic = None

    try:
        diastolic_raw = data.get("diastolic")
        if isinstance(diastolic_raw, float) and not diastolic_raw.is_integer():
            validation_errors.append({"field": "diastolic", "message": "Diastolic pressure must be a whole number"})
            diastolic = None
        else:
            diastolic = int(diastolic_raw)
    except (TypeError, ValueError):
        validation_errors.append({"field": "diastolic", "message": "Diastolic pressure must be a number"})
        diastolic = None

    # Validate blood pressure ranges
    if systolic is not None and not (30 <= systolic <= 250):
        validation_errors.append({"field": "systolic", "message": "Systolic pressure must be between 30-250 mmHg"})

    if diastolic is not None and not (30 <= diastolic <= 250):
        validation_errors.append({"field": "diastolic", "message": "Diastolic pressure must be between 30-250 mmHg"})

    # Validate systolic > diastolic relationship
    if systolic is not None and diastolic is not None and systolic <= diastolic:
        validation_errors.append({"field": "blood_pressure", "message": "Systolic pressure must be greater than diastolic pressure"})
    # Validate heart rate (optional)
    heart_rate = data.get("heart_rate")
    if heart_rate is not None:
        if heart_rate == "" or heart_rate == 0:  # Allow empty string or 0 to be treated as None
            heart_rate = None
        else:
            try:
                if isinstance(heart_rate, float) and not heart_rate.is_integer():
                    validation_errors.append({"field": "heart_rate", "message": "Heart rate must be a whole number"})
                else:
                    heart_rate = int(heart_rate)
                    if not (30 <= heart_rate <= 150):
                        validation_errors.append({"field": "heart_rate", "message": "Heart rate must be between 30-150 bpm"})
            except (TypeError, ValueError):
                validation_errors.append({"field": "heart_rate", "message": "Heart rate must be a number"})
    ...
    ts = datetime.now(timezone.utc) if not ts_raw else datetime.fromisoformat(ts_raw.replace("Z", "+00:00"))
...
def update_record(rec_id: int):
    ...
    validation_errors = []
    new_systolic = rec.systolic
    new_diastolic = rec.diastolic

    if "systolic" in data:
        try:
            systolic_raw = data["systolic"]
            if isinstance(systolic_raw, float) and not systolic_raw.is_integer():
                validation_errors.append({"field": "systolic", "message": "Systolic pressure must be a whole number"})
            else:
                new_systolic = int(systolic_raw)
                if not (30 <= new_systolic <= 250):
                    validation_errors.append({"field": "systolic", "message": "Systolic pressure must be between 30-250 mmHg"})
        except (TypeError, ValueError):
            validation_errors.append({"field": "systolic", "message": "Systolic pressure must be a number"})
    ...
    if "diastolic" in data:
        try:
            diastolic_raw = data["diastolic"]
            if isinstance(diastolic_raw, float) and not diastolic_raw.is_integer():
                validation_errors.append({"field": "diastolic", "message": "Diastolic pressure must be a whole number"})
            else:
                new_diastolic = int(diastolic_raw)
                if not (30 <= new_diastolic <= 250):
                    validation_errors.append({"field": "diastolic", "message": "Diastolic pressure must be between 30-250 mmHg"})
        except (TypeError, ValueError):
            validation_errors.append({"field": "diastolic", "message": "Diastolic pressure must be a number"})

    # Validate systolic > diastolic relationship
    if new_systolic <= new_diastolic:
        validation_errors.append({"field": "blood_pressure", "message": "Systolic pressure must be greater than diastolic pressure"})

    if "heart_rate" in data:
        heart_rate_value = data["heart_rate"]
        if heart_rate_value is None or heart_rate_value == "" or heart_rate_value == 0:
            rec.heart_rate = None
        else:
            try:
                if isinstance(heart_rate_value, float) and not heart_rate_value.is_integer():
                    validation_errors.append({"field": "heart_rate", "message": "Heart rate must be a whole number"})
                else:
                    heart_rate_int = int(heart_rate_value)
                    if not (30 <= heart_rate_int <= 150):
                        validation_errors.append({"field": "heart_rate", "message": "Heart rate must be between 30-150 bpm"})
                    else:
                        rec.heart_rate = heart_rate_int
            except (TypeError, ValueError):
                validation_errors.append({"field": "heart_rate", "message": "Heart rate must be a number"})
    ...
    if "systolic" in data:
        rec.systolic = new_systolic
    if "diastolic" in data:
        rec.diastolic = new_diastolic
    if "tags" in data:
        tags = data.get("tags") or []
        if not isinstance(tags, list):
            return jsonify(error("400", "tags must be a list")), 400
