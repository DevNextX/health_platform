...
def _validate_health_record_data(data):
    """
    Validate health record data according to updated requirements.
    ...
    try:
        ...
        return False, "Invalid systolic pressure", 400
    
    # Validate diastolic pressure
    try:
        diastolic = int(data.get("diastolic"))
    except (TypeError, ValueError):
        return False, "Invalid diastolic pressure", 400
    
    # Updated blood pressure range: 30-250 mmHg
    if not (30 <= systolic <= 250):
        return False, "Systolic pressure must be between 30-250 mmHg", 422
    
    if not (30 <= diastolic <= 250):
        return False, "Diastolic pressure must be between 30-250 mmHg", 422
    
    # New requirement: systolic must be greater than diastolic
    if systolic <= diastolic:
        return False, "Systolic pressure must be greater than diastolic pressure", 422
    
    # Validate heart rate (optional, but if provided must be 30-150 bpm)
    heart_rate = data.get("heart_rate")
    if heart_rate is not None and heart_rate != "":
        try:
            heart_rate = int(heart_rate)
            if not (30 <= heart_rate <= 150):
                return False, "Heart rate must be between 30-150 bpm", 422
        except (TypeError, ValueError):
            return False, "Invalid heart rate", 400
    
    return True, None, None


def _validate_health_record_update(data, current_record):
    """
    Validate health record update data, handling partial updates.
    Returns (is_valid, error_message, error_code).
    Generated by Copilot.
    """
    # Get current values as fallback
    systolic = current_record.systolic
    diastolic = current_record.diastolic
...
def create_record():
    ...
    # Validate health record data using updated validation rules
    is_valid, error_message, error_code = _validate_health_record_data(data)
    if not is_valid:
        return jsonify(error(str(error_code), error_message)), error_code
...
def update_record(rec_id: int):
    ...
    # Validate update data using updated validation rules
    is_valid, error_message, error_code = _validate_health_record_update(data, rec)
    if not is_valid:
        return jsonify(error(str(error_code), error_message)), error_code
