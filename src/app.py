...
from .service.threshold_service import bp as threshold_bp
...
def create_app(config_class: type = Config) -> Flask:
    ...
    with app.app_context():
        ...
        try:
            ...
            try:
                ...
                if 'users' in set(insp2.get_table_names()):
            ...
            if not app.config.get("TESTING", False):
                try:
                    ...
                    # Generated by Copilot: Initialize default thresholds
                    from .models import ThresholdConfig, ThresholdAuditLog
                    if not ThresholdConfig.query.filter_by(status="active").first():
                        import json
                        payload = {
                        ...
                        # Attribute to Super Admin if available
                        sa_user = User.query.filter_by(role="SUPER_ADMIN").first()
                        creator_id = sa_user.id if sa_user else 1
    ...
    # Serve React frontend static files (for unified container deployment)
