  ...
  const columns = [
    { title: '姓名', dataIndex: 'full_name', key: 'full_name', render: (name) => {
      ...
      return (
        <span>
          {name}
          {isSelf && <span style={{ marginLeft: 8, color: '#1890ff' }}>（默认/不可删除）</span>}
        </span>
      );
    } },
      ...
      <Modal title={editing ? '编辑成员' : '新增成员'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null}>
        <Form form={form} layout="vertical" onFinish={onSubmit}>
          ...
          <Form.Item name="age" label="年龄">
            <InputNumber min={0} max={120} style={{ width: '100%' }} placeholder="年龄" />
          </Form.Item>
          <Form.Item name="height" label="身高(cm)">
            <InputNumber min={0} max={300} style={{ width: '100%' }} placeholder="身高(cm)" />
          </Form.Item>
          <Form.Item name="weight" label="体重(kg)">
            <InputNumber min={0} max={500} style={{ width: '100%' }} placeholder="体重(kg)" />
          </Form.Item>
          <Form.Item>
            <Space>
