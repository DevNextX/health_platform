...
const HealthRecords = () => {
  ...
  // Generated by Copilot: Track form validation state for button disabling
  ...
  setEditingRecord(null);
    ...
    handleModalVisibilityChange(true);
    // Validate the form with timestamp to check initial state
    validateInitialForm();
  };
  ...
  const handleEdit = (record) => {
    ...
    handleModalVisibilityChange(true);
    // Validate the populated form to determine initial button state
    validateInitialForm();
  };
  ...
  // Generated by Copilot: Handle form field changes and validation
  const handleFormChange = () => {
    setFieldsChanged(true);
    // Use a small timeout to allow form to update before validation
    setTimeout(() => {
      form.validateFields()
        .then(() => {
  ...
  // Generated by Copilot: Validate initial form state (for editing)
  const validateInitialForm = () => {
    setTimeout(() => {
      form.validateFields()
        .then(() => {
          setFormHasErrors(false);
          setFieldsChanged(true); // Form has valid data, enable button
        })
        .catch(() => {
          setFormHasErrors(true);
          setFieldsChanged(true); // Form has data but invalid, keep button disabled
        });
    }, 100);
  };

  // Generated by Copilot: Reset form state when modal opens/closes
  ...
  {/* Removed extra date chip row to avoid duplication - Generated by Zhuang */}
    ...
    title={editingRecord ? t('health.modal.editTitle') : t('health.modal.addTitle')}
        open={modalVisible}
        onCancel={() => handleModalVisibilityChange(false)}
        footer={null}
        width={600}
      >
        <Form
          ...
          <Form.Item>
            <Space>
              <Button 
                type="primary" 
                htmlType="submit"
                disabled={formHasErrors || !fieldsChanged}
              >
                {editingRecord ? t('health.form.update') : t('health.form.save')}
              </Button>
              <Button onClick={() => handleModalVisibilityChange(false)}>
