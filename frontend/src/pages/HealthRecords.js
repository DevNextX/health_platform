...
const HealthRecords = () => {
  ...
  const [formErrors, setFormErrors] = useState([]); // Generated by Copilot: track validation errors
    ...
    title={editingRecord ? t('health.modal.editTitle') : t('health.modal.addTitle')}
      ...
      >
        <Form
          ...
          onFieldsChange={() => {
            setFormErrors(form.getFieldsError().filter(f => f.errors.length));
          }}
        >
          <Form.Item
            name="timestamp"
      ...
      rules={[{ required: true, message: t('health.form.timestamp') }]}
            ...
            <Col span={12}>
              <Form.Item
                ...
                rules={[
                  ...
                  {
                    validator: (_, value) => {
                      if (value === undefined || value === null || value === '') return Promise.reject(new Error(t('health.form.systolic')));
                      if (isNaN(Number(value))) return Promise.reject(new Error(t('validation.invalidNumber')));
                      const v = Number(value);
                      if (v < 30 || v > 250) return Promise.reject(new Error(t('validation.bloodPressureRange')));
                      const di = form.getFieldValue('diastolic_pressure');
                      if (di !== undefined && di !== null && di !== '' && !isNaN(Number(di))) {
                        if (v <= Number(di)) return Promise.reject(new Error(t('validation.systolicGreaterThanDiastolic')));
                      }
            ...
            <Col span={12}>
              <Form.Item
                ...
                rules={[
                  ...
                  {
                    validator: (_, value) => {
                      if (value === undefined || value === null || value === '') return Promise.reject(new Error(t('health.form.diastolic')));
                      if (isNaN(Number(value))) return Promise.reject(new Error(t('validation.invalidNumber')));
                      const v = Number(value);
                      if (v < 30 || v > 250) return Promise.reject(new Error(t('validation.bloodPressureRange')));
                      const sy = form.getFieldValue('systolic_pressure');
                      if (sy !== undefined && sy !== null && sy !== '' && !isNaN(Number(sy))) {
                        if (Number(sy) <= v) return Promise.reject(new Error(t('validation.systolicGreaterThanDiastolic')));
                      }
                      return Promise.resolve();
                    }
                  }
          ...
          <Form.Item
            ...
            rules={[
              {
                validator: (_, value) => {
                  if (value === undefined || value === null || value === '') return Promise.resolve(); // optional
                  if (isNaN(Number(value))) return Promise.reject(new Error(t('validation.invalidNumber')));
                  const v = Number(value);
                  if (v < 30 || v > 150) return Promise.reject(new Error(t('validation.heartRateRange')));
                  return Promise.resolve();
                }
              }
            ]}
          ...
          <Form.Item>
            <Space>
              <Button type="primary" htmlType="submit" disabled={formErrors.length > 0}>
