...
const HealthRecords = () => {
  ...
  // Track form validation errors to disable submit button
  ...
  const handleEdit = (record) => {
    ...
    setModalVisible(true);
    // Check validation after setting values
    setTimeout(checkFormErrors, 0);
  };
  ...
  // Custom validator for systolic > diastolic relationship. Generated by Copilot for issue #5
  const validateBloodPressureRelation = (rule, value) => {
    if (!value) return Promise.resolve();
    
    const formInstance = form; // Use the form hook instance
    const systolic = formInstance.getFieldValue('systolic_pressure');
    const diastolic = formInstance.getFieldValue('diastolic_pressure');
    
    if (systolic && diastolic && systolic <= diastolic) {
      return Promise.reject(new Error(t('health.validation.systolicGreater')));
    }
    return Promise.resolve();
  };

  // Check form validation errors to enable/disable submit button
  const checkFormErrors = () => {
    form.validateFields()
      .then(() => setHasFormErrors(false))
      .catch(() => setHasFormErrors(true));
  };

  // Watch form changes to update validation state
  const handleFormChange = () => {
    setTimeout(checkFormErrors, 0); // Defer to next tick for accurate validation
  };
    ...
    title={editingRecord ? t('health.modal.editTitle') : t('health.modal.addTitle')}
      ...
      rules={[{ required: true, message: t('health.form.timestamp') }]}
            ...
            <Col span={12}>
              <Form.Item
                ...
                rules={[
                  ...
                  { validator: validateBloodPressureRelation }
                ]}
              >
                <InputNumber
          ...
          <Form.Item
            ...
            rules={[
              { type: 'number', min: 30, max: 150, message: t('health.validation.heartRateRange') }
            ]}
          >
            <InputNumber
          ...
          <Form.Item>
            <Space>
              <Button type="primary" htmlType="submit" disabled={hasFormErrors}>
