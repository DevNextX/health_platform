...
const HealthRecords = () => {
  ...
  // Generated by Copilot: custom validator for systolic > diastolic - simplified
  const validateBloodPressureRelation = (rule, value) => {
    return new Promise((resolve, reject) => {
      ...
      const { systolic_pressure, diastolic_pressure } = formValues;
      
      // Only validate when both values exist and are valid numbers
      if (systolic_pressure && diastolic_pressure) {
        const systolic = Number(systolic_pressure);
        const diastolic = Number(diastolic_pressure);
        
        if (!isNaN(systolic) && !isNaN(diastolic) && systolic <= diastolic) {
          reject(new Error(t('validation.systolicGreaterThanDiastolic')));
          return;
        }
      }
